...
def judge_query_doc_pair_all_doc(judge_gpt, qid, did, query_text, doc_text):
    response = judge_query_doc_pair(judge_gpt, qid, did, query_text, doc_text)
    return extract_judgment(response)
...
def main(args):
    ...
    result_dir=f'{collect_root}/longp_results/{dataset_result_dir}/derived_data/ir_models/llm_ranker_llm_judge_all_doc/model_conf/' + \
    ...
    for qid in tqdm(query_keys[0:sample_qty]):
        ...
        #print(qid, cache_rec is not None)
        if not cache_rec:
            ...
            for did in runs[qid]:
                ...
                score = judge_query_doc_pair_all_doc(judge_gpt, qid, did, query_text, doc_text) + \
                                                     np.random.uniform(0, MAX_RAND)
                run.append((did, score))

            t1 = time()

            cache_rec = dict(query_id = qid, time=t1 - t0, run=run)
